...
def cleanup(repos):
    ...
    for i, repo_name in enumerate(
        list(repos.keys())[_offset : min(_offset + n, len(repos))]
    ):
        if i != 0:
            time.sleep(5)
...
logging.info("Removing superfluous repos.")
cleanup(previous_repos)
logging.info("Removing superfluous skips.")
cleanup(previous_skips)

store_data(previous_repos, "data.json")
store_data(previous_skips, "skips.json")
